...
\subsection{Décomposition en work package}
...
\begin{itemize}
    \item \textbf{Hardware} : Jules Noumba s'occupera de l'agencement sur le piège camera, des différents capteurs qui permettrons de récupérer les métadonnée lié au photos.
    \item \textbf{\'Energie} : Nolan Buchet aura pour rôle de gérer les consommations d'énergie des composants agencé par Jules Noumba.
    \item \textbf{Transmission} : Joris Menard s'occupera de la Transmission sans fils des données entre le piège caméra et l'unité de stockage distante.
    \item \textbf{Stockage des données} : Davy Clark s'occupera de trouver une solution afin de stocker les données récupérer par le piège caméra et de la développer.
    ...
    \item \textbf{Communication avec le client} : Ewen Le Callet aura également le rôle d'interlocuteur entre l'équipe de projet et le client afin de clarifier certains points ou de tenir compte de l'avancer du projet
\end{itemize}
...
\begin{itemize}
    \item \textbf{Interface Homme Machine (IHM)} : Xueying Xu aura le rôle de développer une interface homme machine (IHM), pour l'utilisateur du produit final puisse visualiser les données récupérer par le piège caméra.
    \item \textbf{IA} : Ewen Le Callet s'occupera de mettre en place l'intelligence artificielle afin de réaliser une identification des animaux sur les photos prisent par le piège caméra.
\end{itemize}
